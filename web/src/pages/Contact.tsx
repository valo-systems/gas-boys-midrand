  ...
  return (
    <div className="pt-24 min-h-screen">
      <div className="max-w-7xl mx-auto px-6 md:px-12 pb-20">
        <div className="section-label mb-2">{s.address}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div>
            <div className="space-y-5 mb-8">
              {[
                { Icon: MapPin,    label: 'Address', val: s.address },
                { Icon: Phone,     label: 'Phone',   val: s.phone },
                { Icon: Envelope,  label: 'Email',   val: s.email },
                { Icon: Clock,     label: 'Hours',   val: `Mon–Fri: ${s.hours.weekdays}  ·  Sat: ${s.hours.saturday}  ·  Sun: ${s.hours.sunday}` },
              ].map(({ Icon, label, val }) => (
                <div key={label} className="flex gap-4">
                  ...
                  <div>
                    <div className="text-xs font-semibold text-gas-muted uppercase tracking-wider mb-0.5">{label}</div>
                    <div className="text-sm text-gas-text2">{val}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="card h-56 flex items-center justify-center gap-3 text-gas-muted mb-5">
              ...
              <a href={s.mapUrl} target="_blank" rel="noopener noreferrer" className="text-sm hover:text-yellow-500 transition-colors">
                View on Google Maps →
              </a>
            </div>

            <a
              href={`https://wa.me/${s.whatsapp}?text=Hi%20Gas%20Boys!%20I%27d%20like%20to%20get%20in%20touch.`}
            ...
            {submitted ? (
              ...
              <div className="card p-8 space-y-4">
                ...
                <div>
                  ...
                  <input className="input" value={form.contact} onChange={e => update('contact', e.target.value)} placeholder="How should we reach you?" />
                </div>
                <div>
                  <label className="label">Subject</label>
                  <select className="input" value={form.subject} onChange={e => update('subject', e.target.value)}>
                    <option>Gas Delivery</option>
                    <option>Repair Booking</option>
                    <option>Installation Quote</option>
                    <option>CoC Certificate</option>
                    <option>Bulk / Commercial</option>
                    <option>General Enquiry</option>
                  </select>
                </div>
                <div>
                  <label className="label">Message</label>
                  <textarea className="input resize-none" rows={5} value={form.message} onChange={e => update('message', e.target.value)} placeholder="How can we help you?" />
                </div>
                <button
                  onClick={() => form.name && form.contact && form.message && setSubmitted(true)}
